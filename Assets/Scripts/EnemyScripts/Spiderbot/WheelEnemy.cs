...
public class WheelEnemy : BaseEnemy {
	...
	void Update () {
		...
		if (true) {
			GraphicalObject.transform.RotateAround(new Vector3(GraphicalObject.transform.position.x,GraphicalObject.transform.position.y,GraphicalObject.transform.position.z),Vector3.right,wheel.rpm);
		}
	}
